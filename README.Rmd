...
![](man/figures/pipeline-vis.png){width=50%}
...
One of the benefits of {sewage} is that we can pick the pipeline up at any point using the optional `start` and `halt` options in `run`. This means we can build an entire data pipeline, but test small sections. For example, we may want to pass data down one branch without running the entire pipeline.
...
draw(pipeline)
```
![](man/figures/pipeline3.png){width=50%}
...
Using these three components (`Nodes`, `Splitters` and `Joiners`) you can construct complex data pipelines and run them in a single call. By leveraging the `start` and `halt` arguments not only can we construct large complex data pipelines, but we can isolate components of the pipeline making data scripts more modular and easier to maintain. 
